...
const ensureOpportunityExists = async (opportunityId: string) => {
  await service.getById(opportunityId);
};

const getOpportunityAudioDir = (opportunityId: string) =>
  path.resolve(process.cwd(), audioBaseRelative, opportunityId);

const listOpportunityAudios = async (opportunityId: string): Promise<OpportunityAudioItem[]> => {
  ...
  try {
    ...
    const files = await Promise.all(
      names.map(async (name) => {
        ...
        } satisfies OpportunityAudioItem;
      }),
    );

    return files
      .filter((item): item is OpportunityAudioItem => Boolean(item))
...
constructionOpportunitiesV2Router.post("/:id/audios", audioUpload.single("audio"), async (req, res) => {
  ...
  const [created] = await listOpportunityAudios(opportunityId).then((items) =>
    items.filter((item) => item.relativePath.endsWith(`/${filename}`)),
  );
