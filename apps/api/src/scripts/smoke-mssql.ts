...
async function main() {
  ...
  const opp = await sequelize.query<{ total: number }>(
    `SELECT COUNT(1) AS total FROM ${env.sqlServerSchema}.ConstructionOpportunity WHERE isDeleted = 0`,
    { type: QueryTypes.SELECT },
  );
  const photos = await sequelize.query<{ total: number }>(
    `SELECT COUNT(1) AS total FROM ${env.sqlServerSchema}.ConstructionOpportunityPhoto`,
    { type: QueryTypes.SELECT },
  );
