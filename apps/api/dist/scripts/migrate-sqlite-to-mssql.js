...
const createSequelize = () => {
    ...
    const password = process.env.DB_PASSWORD_B64
    ...
    const trust = (process.env.SQLSERVER_TRUST_SERVER_CERTIFICATE ?? process.env.DB_TRUST_SERVER_CERTIFICATE ?? "true") ===
        "true";
    if (!password) {
...
const assertSchemaReady = async (sequelize) => {
    const rows = await sequelize.query(`SELECT t.name AS name
     FROM sys.tables t
     INNER JOIN sys.schemas s ON s.schema_id = t.schema_id
     WHERE s.name = :schema
     ORDER BY t.name`, { replacements: { schema: SCHEMA }, type: QueryTypes.SELECT });
    const found = new Set(rows.map((r) => r.name));
    console.log(`[etl] tabelas em ${SCHEMA}:`, [...found].join(", ") || "(nenhuma)");
    const missing = TARGET_TABLES.filter((name) => !found.has(name));
    if (missing.length > 0) {
        throw new Error(`Tabelas faltando no schema ${SCHEMA}: ${missing.join(", ")}. ` +
            `Rode docs/sql/fx_obras_schema_2008r2.sql no database ${process.env.DB_DATABASE ?? "?"} ` +
            `(USE correto + schema fx_obras).`);
    }
};
const countTarget = async (sequelize, table) => {
    const rows = await sequelize.query(`SELECT COUNT(1) AS total FROM ${q(table)}`, { type: QueryTypes.SELECT });
    return Number(rows[0]?.total ?? 0);
};
const truncateTarget = async (sequelize) => {
...
async function main() {
    ...
    const [opportunities, photos, history, providers, links,] = await Promise.all([
    ...
    try {
        ...
        for (const row of providers) {
            await sequelize.query(`INSERT INTO ${q("ServiceProvider")} (
        ...
        for (const row of opportunities) {
            await sequelize.query(`INSERT INTO ${q("ConstructionOpportunity")} (
        ...
        for (const row of photos) {
            await sequelize.query(`INSERT INTO ${q("ConstructionOpportunityPhoto")} (
        ...
        for (const row of history) {
            await sequelize.query(`INSERT INTO ${q("ConstructionOpportunityHistory")} (
        ...
        for (const row of links) {
            await sequelize.query(`INSERT INTO ${q("OpportunityServiceProvider")} (
