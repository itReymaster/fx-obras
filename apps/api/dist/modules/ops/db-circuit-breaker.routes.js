...
import { clearSqlDialectCircuitOverride, getSqlDialectCircuitStatus, setActiveSqlDialect, } from "../../shared/database/sql-dialect-circuit-breaker.js";
/**
...
const requireBreakerToken = (req) => {
    const expectedRaw = process.env.CIRCUIT_BREAKER_TOKEN;
    const expected = expectedRaw ? stripQuotes(expectedRaw) : "";
    if (!expected) {
        throw new AppError("Circuit breaker desabilitado. Defina CIRCUIT_BREAKER_TOKEN no ambiente.", 503);
    }
    ...
    if (!provided || provided !== expected) {
        throw new AppError("Token do circuit breaker invalido. Use Authorization: Bearer <CIRCUIT_BREAKER_TOKEN> (no PowerShell prefira curl.exe).", 401);
    }
};
...
dbCircuitBreakerRouter.post("/db-circuit", (req, res) => {
    ...
    const payload = z
        ...
        .parse(req.body);
    const status = setActiveSqlDialect(payload.dialect, payload.reason);
    res.json({
