...
const breakerFilePath = () => process.env.SQL_DIALECT_BREAKER_FILE?.trim() ||
...
const normalize = (value) => {
    const normalized = String(value ?? "")
        .trim()
        .toLowerCase();
    if (normalized === "sqlite" || normalized === "mssql")
        return normalized;
    return null;
};
const readFileOverride = () => {
    try {
 ...
 */
export const getActiveSqlDialect = () => {
...
export const setActiveSqlDialect = (dialect, reason) => {
